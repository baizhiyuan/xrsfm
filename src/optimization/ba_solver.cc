...
void AddCovisibilityEdge(ceres::Problem &problem, Map &map,
    ...
    for (auto &frame : map.frames_) {
        ...
        for (const auto &cor_id : map.frameid2covisible_frameids_[frame.id]) {
            ...
            ceres::CostFunction *cost_function =
                new PoseGraphCost(q_mea, p_mea, weight_o);
...
void AddLoopEdge(ceres::Problem &problem, Map &map, const LoopInfo &loop_info,
                 std::vector<double> &s_vec, std::vector<double> &s_vec_loop,
                 std::vector<Pose> &twc_vec, std::vector<int> &num_cov,
                 double weight_o, bool use_key) {
...
void BASolver::ScalePoseGraphUnorder(const LoopInfo &loop_info, Map &map,
    ...
    // covisibility edge
    AddCovisibilityEdge(problem, map, s_vec, twc_vec, num_cov, weight_o,
                        use_key);
    // loop edge
    AddLoopEdge(problem, map, loop_info, s_vec, s_vec_loop, twc_vec, num_cov,
                weight_o, use_key);
    ...
    if (!use_key) {
        for (size_t i = 0; i < num_frames; ++i) {
            if (i % (num_frames / 10) == 0 || s_vec[i] < 0)
...
std::vector<int> FindLocalBundle(const int frame_id, Map &map,
    ...
    std::vector<std::pair<int, int>> cov_vec(covisiblity.begin(),
                                             covisiblity.end());
    ...
    const std::array<std::pair<double, double>, 8> selection_thresholds = {
        std::make_pair(min_tri_angle_rad / 1.0, 0.6 * num_p3d),
        std::make_pair(min_tri_angle_rad / 1.5, 0.6 * num_p3d),
        std::make_pair(min_tri_angle_rad / 2.0, 0.5 * num_p3d),
        std::make_pair(min_tri_angle_rad / 2.5, 0.4 * num_p3d),
        std::make_pair(min_tri_angle_rad / 3.0, 0.3 * num_p3d),
        std::make_pair(min_tri_angle_rad / 4.0, 0.2 * num_p3d),
        std::make_pair(min_tri_angle_rad / 5.0, 0.1 * num_p3d),
        std::make_pair(min_tri_angle_rad / 6.0, 0.1 * num_p3d)};
    ...
    for (const auto &selection_threshold : selection_thresholds) {
        for (size_t frame_id = 0; frame_id < cov_vec.size(); ++frame_id) {
            if (cov_vec[frame_id].second < selection_threshold.second)
                break;
...
std::vector<int> CovisibilityNeibors(const int frame_id, Map &map,
    ...
    for (const auto &[t_frame_id, num_cov] : cov_vec) {
        local_bundle_image_ids.push_back(t_frame_id);
        if (local_bundle_image_ids.size() == num_images)
            break;
    }
    return local_bundle_image_ids;
}
